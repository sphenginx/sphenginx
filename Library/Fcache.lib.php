 ...
 * 无论你是读取多大的数据，都需要从磁盘读出整个文件到内存，然后解析，获取你要的部分数据；
...
$cache->add('FCache', $storeData);
$cache->add('FCache', $storeData);
$cache->add('FCache', $storeData);
$cache->add('FCache', $storeData);

print_r($storeData = $cache->get('FCache'));

$cache->delete('FCache');

print_r ($cache->get('FCache') ? 'exist': 'has no cache');

$cache->add('FCache1;', 123);
$cache->add('FCache2;', 234);
$cache->add('FCache3;', 345);
$cache->flush();

print_r ($cache->get('FCache3;') ? 'exist': 'has no cache');